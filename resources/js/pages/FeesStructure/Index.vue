...
onMounted(async () => {
    try {
        const response = await fetch('/api/modules/fees_structure');
...
<template>
    <AppLayout>
        <PageHeadIndi heading="Fees Structure" />
            ...
            <div v-else class="space-y-12">
                <div v-for="(file, index) in moduleData.files" :key="index">
                    ...
                    <div v-if="file.type === 'pdf'" class="w-full h-[calc(100vh-200px)] rounded-xl overflow-hidden border shadow-sm bg-white">
                        <iframe :src="file.url || PdfFile" class="w-full h-full border-0"></iframe>
                    </div>
