...
import uniformPdf from '@/../../resources/pdfs/SummerUniform.pdf';
...
onMounted(async () => {
    try {
        const response = await fetch('/api/modules/school_uniform');
            ...
            <div v-else class="space-y-12">
                <div v-for="(file, index) in moduleData.files" :key="index">
                    ...
                    <div v-if="file.type === 'pdf'" class="w-full h-[calc(100vh-200px)] rounded-xl overflow-hidden border shadow-sm bg-white">
                        <iframe :src="file.url || uniformPdf" class="w-full h-full border-0"></iframe>
                    </div>
